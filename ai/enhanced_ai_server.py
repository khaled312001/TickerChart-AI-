...
class EnhancedAIServer:
    ...
    def generate_market_data(self, days=30):
        ...
        prices = 10885.58 + np.cumsum(np.random.randn(days) * 0.5)
...
@app.route('/')
def home():
    return jsonify({"status": "Enhanced AI Server Running", "timestamp": datetime.now().isoformat()})
    ...
    try:
        ...
        for item in portfolio:
            ...
            if sector not in sectors:
                sectors[sector] = 0
            sectors[sector] += item.get('value', 0)
...
if __name__ == '__main__':
    ...
    print(f"Starting Enhanced AI Server on port {port}")
...
if __name__ == '__main__': 
    print(f"Starting Enhanced AI Server on port {os.environ.get('PYTHON_PORT', 8001)}") 
